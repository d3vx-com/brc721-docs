...
- Download and top up the [OrdinalSafe wallet](https://ordinalsafe.xyz/) with at least 40000 SATS (0.0004BTC). It is the only wallet that supports BRC721 at this moment, more integrations are in the pipeline.
...
First, as the collection creator, we will inscribe one token for ourselves. The exact process will apply to every access list address inscription, but the form should have the price field populated. In our case, it is empty to mint for free. But if you to sell the token, you can specify the price in SATS.
